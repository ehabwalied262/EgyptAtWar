...
<div className="flex flex-col items-center min-h-screen p-4 sm:p-8"> <style> {` .channels-wrapper { width: 100%; max-width: 1200px; /* Max width for larger screens */ position: relative; } .intro-text { text-align: center; margin-bottom: 40px; color: white; } .channels-container { display: flex; flex-direction: column; gap: 30px; /* Space between rows */ align-items: flex-end; /* Align rows to the right */ width: 100%; } .channel-row { display: flex; gap: 15px; /* Space between avatars in a row */ padding: 5px; border: 1px solid rgba(255, 255, 255, 0.1); /* Subtle border for separation */ border-radius: 5px; background: rgba(255, 255, 255, 0.05); /* Light background for contrast */ } .channel-item { display: flex; flex-direction: column; align-items: center; position: relative; } .channel-avatar { width: 30px; height: 30px; border-radius: 50%; /* Circular shape */ object-fit: cover; transition: all 0.4s ease; } .hovered-container { position: fixed; left: 20px; top: 45%; transform: translateY(-50%); width: 400px; /* Fixed width for consistency */ background: rgba(0, 0, 0, 0.8); /* Semi-transparent black background */ border: 1px solid rgba(255, 255, 255, 0.2); /* Subtle border */ border-radius: 10px; padding: 15px; display: flex; align-items: center; opacity: 0; transition: opacity 0.3s ease; } .hovered-container.visible { opacity: 1; } .hovered-container .channel-avatar { width: 70px; height: 70px; border-radius: 50%; /* Keep circular shape */ box-shadow: 0 0 15px rgba(255, 255, 255, 0.5); /* Glow effect */ margin-right: 15px; } .hovered-content { display: flex; flex-direction: column; flex: 1; } .hovered-channel-name { color: white; font-size: 16px; font-weight: bold; margin-bottom: 5px; } .description-box { color: white; font-size: 16px; display: flex; align-items: center; } .channel-name { color: white; font-size: 10px; margin-top: 6px; text-align: center; max-width: 60px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .typing-cursor::after { content: '|'; animation: blink 0.8s infinite; margin-left: 4px; } @keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0; } } @media (max-width: 640px) { .channels-wrapper { max-width: 100%; } .intro-text { margin-bottom: 20px; } .intro-text h2 { font-size: 1.5rem; /* Smaller heading on mobile */ } .intro-text p { font-size: 0.875rem; /* Smaller paragraph on mobile */ } .channels-container { align-items: center; /* Center on mobile */ gap: 20px; /* Larger gap between rows for clarity */ } .channel-row { gap: 10px; /* Space between avatars */ padding: 5px; flex-wrap: wrap; /* Allow avatars to wrap to next line */ justify-content: center; /* Center avatars in the row */ width: 100%; /* Full width */ max-width: 300px; /* Limit row width for better layout */ border: 1px solid rgba(255, 255, 255, 0.2); /* Slightly more visible border */ border-radius: 5px; background: rgba(255, 255, 255, 0.1); /* Slightly darker background */ } .channel-avatar { width: 24px; height: 24px; } .channel-name { font-size: 8px; max-width: 50px; } .hovered-container { position: static; /* Remove fixed positioning */ width: 90%; /* Responsive width */ max-width: 300px; padding: 8px; margin: 20px auto 0; /* Center and add margin on top */ opacity: 1; /* Always visible when hovered */ } .hovered-container .channel-avatar { width: 40px; height: 40px; margin-right: 10px; } .hovered-channel-name { font-size: 12px; } .description-box { font-size: 12px; } } `} </style>
{/* Channels Wrapper */}

<div className="channels-wrapper"> {/* Introductory Text (above the channels) */} <div className="intro-text"> <h2 className="text-3xl font-bold mb-4">Explore Our Curated YouTube Channels</h2> <p className="text-lg text-gray-300">Discover educational and inspiring content!</p> </div>
{/* Channels Container */}

<div className="channels-container"> {(window.innerWidth <= 640 ? rowsMobile : rowsDesktop).map((row, rowIndex) => ( <div key={rowIndex} className="channel-row"> {row.map((channel) => ( <div key={channel.id} className="channel-item" onMouseEnter={() => setHoveredChannel(channel)} onMouseLeave={handleMouseLeave} > <a href={channel.channelLink} target="_blank" rel="noopener noreferrer"> <img src={channel.channelAvatar} alt={channel.channelName} className="channel-avatar" /> </a> <span className="channel-name">{channel.channelName}</span> </div> ))} </div> ))} </div>
{/* Hovered Container (appears below on mobile) */}
{hoveredChannel && (

<div className={`hovered-container ${hoveredChannel ? 'visible' : ''}`}> <a href={hoveredChannel.channelLink} target="_blank" rel="noopener noreferrer"> <img src={hoveredChannel.channelAvatar} alt={hoveredChannel.channelName} className="channel-avatar" /> </a> <div className="hovered-content"> <div className="hovered-channel-name">{hoveredChannel.channelName}</div> <div className="description-box"> <span className="typing-cursor">{typedText}</span> </div> </div> </div> )} </div> </div> ); };
